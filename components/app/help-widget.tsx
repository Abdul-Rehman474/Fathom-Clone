              ...
              {view === 'home' && (
                <div className="space-y-3">
                  ...
                  <QuickReply
                    ...
                    label="Ask the help bot"
                    onClick={() => setView('ai')}
                  />
                  <QuickReply
                    icon={<Mail className="size-4" />}
                    label="Open a ticket (reply within a day)"
...
function AskAi({ onBack }: { onBack: () => void }) {
  ...
  async function send() {
    ...
    try {
      ...
      const json = (await res.json()) as { answer?: string; message?: string };
      setA(json.answer ?? json.message ?? 'Sorry, I’m not sure.');
    } catch {
