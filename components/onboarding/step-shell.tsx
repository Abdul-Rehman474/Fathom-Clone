...
export function StepShell({
  ...
  return (
    <div className="relative min-h-screen bg-bg-app">
      ...
      <main className="page-in mx-auto flex max-w-[900px] flex-col items-center px-6 pb-32 pt-12 text-center">
        <p className="micro-label mb-6">
          Step {step} of {ONBOARDING_TOTAL} · {label}
        </p>
        {children}
      </main>

      <footer className="fixed inset-x-0 bottom-0 border-t border-border bg-bg-app px-4 py-4 text-center text-sm text-text-3">
        Signing up as <span className="text-text-2">{email}</span>. Wrong account?{' '}
        <button onClick={signOut} className="text-lime hover:underline">
          Sign out
        </button>
      </footer>
    </div>
