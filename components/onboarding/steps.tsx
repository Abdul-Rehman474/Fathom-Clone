...
const continueBtn = 'h-11 w-[450px] max-w-full rounded-btn font-semibold transition-colors';
  ...
  return (
    <div className="w-full">
      ...
      <div className="mx-auto mt-12 grid max-w-3xl gap-4 text-left sm:grid-cols-2">
        <form
          action={saveAccountType}
          className="flex flex-col gap-4 rounded-card border border-border bg-surface-1 p-6"
        >
        ...
        <form
          action={saveAccountType}
          className="flex flex-col gap-4 rounded-card border border-lime/50 bg-surface-1 p-6"
        >
          ...
          <ul className="space-y-2 text-sm text-text-2">
            <li>
              <span className="text-lime">✓</span> Set individual and team preferences
            </li>
            <li>
              <span className="text-lime">✓</span> Create private and shared workspaces
            </li>
            <li>
              <span className="text-lime">✓</span> Add teammates anytime
            </li>
          </ul>
          <SubmitButton
            pendingLabel="Saving…"
  ...
  return (
    <form action={savePreferences} className="w-full">
      ...
      <p className="mx-auto mt-8 max-w-2xl text-xl leading-relaxed text-text-1">
        Take notes on <SentenceSelect value={notes} onValueChange={setNotes} options={NOTES_ON} /> and share with{' '}
      ...
      <label className="mx-auto mt-10 flex max-w-xl cursor-pointer items-start gap-3 text-left text-sm text-text-2">
        ...
        <span>
          I understand I&apos;m responsible for collecting attendee consent for the recording and transcription, in
      ...
      <div className="mt-8 flex justify-center">
        <SubmitButton
          pendingLabel="Saving…"
          disabled={!consent}
          className={cn(continueBtn, consent ? enabled : disabled)}
        >
          Continue
        </SubmitButton>
      </div>
  ...
  return (
    <form action={saveAboutYou} className="w-full">
      ...
      <p className="mx-auto mt-8 max-w-2xl text-xl leading-relaxed text-text-1">
        ...
        />{' '}
        as <SentenceSelect value={rol} onValueChange={setRol} options={toOpts(ROLES)} placeholder="Select Your Role" />.
      </p>
...
export function UsageStep({ usage }: { usage: string }) {
  ...
  return (
    <form action={saveUsage} className="w-full">
      ...
      <h1 className="font-display text-3xl font-semibold tracking-tight sm:text-5xl">
        How are you planning to use {BRAND_NAME}?
      </h1>
      ...
      <div className="mt-10 flex justify-center">
        <SubmitButton
          pendingLabel="Saving…"
          disabled={!choice}
          className={cn(continueBtn, choice ? enabled : disabled)}
        >
          Continue
        </SubmitButton>
      </div>
  ...
  return (
    <div className="w-full">
      ...
      <div className="mt-10 flex flex-col items-center gap-3">
        ...
        <form action={advanceToFirstCall}>
          <SubmitButton className="text-sm text-text-3 hover:text-text-2">Skip this step</SubmitButton>
        </form>
      </div>
      <p className="mx-auto mt-8 max-w-lg text-sm text-text-3">
        Don&apos;t worry, {BRAND_NAME} will only join the meetings that you ask it to. You&apos;re in control here.
      </p>
    </div>
  );
}
    ...
    <div className="w-full">
      ...
      <div className="mt-8">
        <form action={finishOnboarding}>
          <SubmitButton className="text-cyan hover:underline">Go to My Calls →</SubmitButton>
        </form>
      </div>
      <p className="mt-6 text-sm text-text-3">{BRAND_NAME} runs in your browser. There is nothing to install.</p>
    </div>
  );
}
